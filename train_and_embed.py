...
def get_classifier(model_name, n_classes=2, use_bfloat16=False, device="cuda"):
    ...
    model = AutoModelForSequenceClassification.from_pretrained(model_name, num_labels=n_classes, torch_dtype=torch_dtype).to(device)
...
def get_cls_bertlike(x):
    cls_emebds = x[:, 0, :]     # get representation associated with the "CLS" token. In BERT-like models this is usually assigned with the first token of the input sequence (idx=0)
    return cls_emebds
...
def main(args):
    ...
    if args.num_classes != n_inferred_classes:
        print(f"number of inferred target classes ({n_inferred_classes}) != number of given target classes ({args.num_classes})")
    ...
    for split in splits:
        ...
        split_logits, split_last_hiddens = embed(model, tokenizer, data=split_data, selection_strategy=get_cls_bertlike, args=args)
